...
$$v' = q v_\perp + p p^{-1} v_\parallel \quad \tag{7}$$

然而，这种形式两侧不对称。为了寻求形式对称且无歧义的表达式，我们引入了**角度半角拆分（Half-Angle Decomposition）**。
...
### 5.3 替换与推导三维夹心乘积 (Sandwich Product)

将 $q = p p$ 代入公式 (7)：

$$v' = p p v_\perp + p p^{-1} v_\parallel = p \left( p v_\perp + p^{-1} v_\parallel \right) \quad \tag{8}$$
...
- $v_\perp = [0, \vec{v}_\perp]$，由于 $\vec{u} \cdot \vec{v}_\perp = 0$：
  ...
  $$v_\perp p^* = \left[ \sin\frac{\theta}{2} (\vec{v}_\perp \cdot \vec{u}), \; \cos\frac{\theta}{2}\vec{v}_\perp - \sin\frac{\theta}{2} (\vec{v}_\perp \times (-\vec{u})) \right] = \left[ 0, \; \cos\frac{\theta}{2}\vec{v}_\perp + \sin\frac{\theta}{2} (\vec{u} \times \vec{v}_\perp) \right]$$

  因此得出重要交换关系：

  $$p v_\perp = v_\perp p^* \implies p v_\perp p^* = p v_\perp p^{-1}$$

同理分析平行分量 $v_\parallel = [0, \vec{v}_\parallel]$（因为 $\vec{u} \parallel \vec{v}_\parallel \implies \vec{u} \times \vec{v}_\parallel = \vec{0}$）：

$$p v_\parallel = \left[ -\sin\frac{\theta}{2} (\vec{u} \cdot \vec{v}_\parallel), \; \cos\frac{\theta}{2} \vec{v}_\parallel \right] = v_\parallel p$$

结合 $p p^{-1} = 1$，可知平行分量满足 $p^{-1} v_\parallel = v_\parallel p^*$。

最终统一将垂直与平行分量综合整合，导出了三维空间四元数旋转的最终**双边夹心乘积公式**：

$$\mathbf{v' = p v p^*} \quad \text{或} \quad \mathbf{v' = p v p^{-1}}$$
